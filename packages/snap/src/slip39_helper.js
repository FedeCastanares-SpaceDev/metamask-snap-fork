...
function splitSecret(threshold, shareCount, sharedSecret) {
  ...
  if (threshold === 1) {
    return slip39Generate(Array(), shareCount, () => sharedSecret);
  }
...
function getSalt(identifier) {
  const salt = slip39EncodeHex(SALT_STRING);
...
function rs1024VerifyChecksum(data) {
  return rs1024Polymod(slip39EncodeHex(SALT_STRING).concat(data)) === 1;
}
